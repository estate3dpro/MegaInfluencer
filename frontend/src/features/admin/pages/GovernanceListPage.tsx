...
export type GovernanceListConfig = {
  title: string;
  description: string;
  singular: string;
  icon: LucideIcon;
...
export function GovernanceListPage({ config }: { config: GovernanceListConfig }) {
        ...
        <CardContent className="overflow-x-auto border-t p-0">
          <table className="w-full min-w-[740px] text-left text-sm">
            <thead className="border-b bg-muted/40 text-xs text-muted-foreground">
              <tr>
                <th className="px-5 py-3 font-medium">{config.singular}</th>
