...
export async function getStoreProducts(page = 1, options: { search?: string; all?: boolean } = {}) {
  return (await apiClient.get<{ products: StoreProduct[]; pagination: Pagination }>("/store/products", { params: { page, limit: 25, ...options } })).data;
}
