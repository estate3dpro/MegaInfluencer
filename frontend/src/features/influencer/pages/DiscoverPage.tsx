...
export function DiscoverPage() {
  ...
  const campaigns = useMemo(
    () => (query.data ?? []).filter((item) =>
      `${item.title} ${item.category} ${item.organization?.name ?? ""}`.toLowerCase().includes(search.toLowerCase()),
    ),
    [query.data, search],
  );
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <Card className="border-primary/20 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent shadow-card">
        <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
          ...
          <div><p className="font-display text-lg font-semibold">Opportunities matched to creators</p><p className="mt-1 text-sm text-muted-foreground">Review a campaign before applying. Brands are notified as soon as you send your request.</p></div>
        </CardContent>
      </Card>
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{campaigns.map((campaign) => <Opportunity key={campaign.id} campaign={campaign} onPreview={() => setPreview(campaign)} onApply={() => setApplyingTo(campaign)} />)}</section>
  ...
  return (
    <Card className="group overflow-hidden shadow-card transition-shadow hover:shadow-lg">
      ...
      <CardContent className="p-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2"><Button variant="outline" onClick={onPreview}><Eye className="h-4 w-4" />Preview</Button>{applied?.status === "ACCEPTED" ? <Button asChild><Link to="/influencer/products" search={{ campaignId: campaign.id, campaignTitle: campaign.title }}>Go to products <ArrowUpRight className="h-4 w-4" /></Link></Button> : applied ? <Button disabled variant="outline">{applied.status}</Button> : <Button onClick={onApply}>Apply <ArrowUpRight className="h-4 w-4" /></Button>}</div>
      </CardContent>
    </Card>
  );
}
